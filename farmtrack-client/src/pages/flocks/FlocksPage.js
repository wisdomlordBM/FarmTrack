...
export default function FlocksPage() {
  ...
  const handleConfirmDeactivate = async () => {
    try {
      await API.put(`/flock/${confirmModal.id}/deactivate`);
  ...
  return (
    <div>
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {paginatedFlocks.map((f, i) => {
              const survivalRate = f.totalBirds > 0 ? Math.round((f.aliveBirds / f.totalBirds) * 100) : 0;
              return (
                <motion.div key={f.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                  transition={{ delay: i * 0.08 }}
                  className="bg-white rounded-3xl border border-slate-200 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all overflow-hidden">
                  <div className="bg-gradient-to-r from-emerald-600 to-emerald-500 px-5 py-4">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${
                        f.status === 'Active' ? 'bg-white text-emerald-600' : 'bg-white/20 text-white'
                      }`}>{f.status}</span>
                    </div>
                  ...
                  <div className="p-5">
                    ...
                    <div className="mb-4">
                      <div className="flex justify-between text-xs text-slate-500 mb-1.5">
                        ...
                        <span className={`font-bold ${survivalRate >= 80 ? 'text-emerald-600' : 'text-rose-500'}`}>{survivalRate}%</span>
                      </div>
                      <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
                        <motion.div initial={{ width: 0 }} animate={{ width: `${survivalRate}%` }}
                          transition={{ delay: 0.3, duration: 0.8 }}
                          className={`h-full rounded-full ${survivalRate >= 80 ? 'bg-emerald-500' : survivalRate >= 60 ? 'bg-amber-500' : 'bg-rose-500'}`} />
                      </div>
                    </div>
                    ...
                    {f.status === 'Active' && (
                      <button onClick={() => setConfirmModal({ isOpen: true, id: f.id })}
            ...
            <motion.div className="bg-white rounded-[2rem] w-full max-w-md shadow-2xl shadow-slate-300/40"
              ...
              <form onSubmit={handleSubmit} className="p-7 space-y-5">
                <div>
                  ...
                  <input placeholder="e.g. Batch A — Jan 2025" value={form.batchName}
                ...
                <div>
                  ...
                  <select value={form.birdType} onChange={e => setForm({ ...form, birdType: e.target.value })}
                    ...
                    <option>Layer</option><option>Broiler</option><option>Cockerel</option>
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-2">Total Birds</label>
                  <input type="number" placeholder="e.g. 500" value={form.totalBirds}
      ...
      <ConfirmModal
        ...
        title="Deactivate Flock?"
        message="This flock will be marked as inactive. You can still see its records but it won't appear in active lists."
